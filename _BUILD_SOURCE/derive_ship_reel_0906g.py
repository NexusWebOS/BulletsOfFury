...
⚠ WHAT IS APPROXIMATED, STATED PLAINLY: past 90 degrees a real aircraft shows its UNDERSIDE, and a
plate only has a topside. The second half of the roll is the MIRRORED plate darkened by UNDER_DIM,
which reads as the belly coming round; it is not authored underside art. If Mike wants a true
underside the answer is an authored plate for it, not a better transform.
...
def main():
    ...
    for i in range(8):
        frames['_br%d' % i] = roll_frame(plate, i * 45)
