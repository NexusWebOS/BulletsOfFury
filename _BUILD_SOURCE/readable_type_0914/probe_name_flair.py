...
R=Path(__file__).resolve().parents[2];O=R/'_shots/pilot_name_flair_0914'
...
with sync_playwright()as p:
 ...
 # Whole-name plate colors and the actual dialogue renderer, all nine pilots.
 pg.evaluate("""()=>{ctx.save();ctx.setTransform(2,0,0,2,0,0);ctx.fillStyle='#102033';ctx.fillRect(0,0,VW,VH);PILOTS.forEach((p,i)=>{pilotNameDraw(p.name,VW/2,32+i*55,22,p.tint,1);});ctx.restore();}""");shot('all_pilot_names')
 pg.evaluate("""()=>{run.pilot='yuri';ctx.save();ctx.setTransform(2,0,0,2,0,0);ctx.fillStyle='#102033';ctx.fillRect(0,0,VW,VH);dlgBox({who:'YURI',full:'INCOMING FIGHTERS! LOCK YOUR MISSILES ONTO THE ENEMY AND KEEP MOVING.',screenSpace:false,y:180});ctx.restore();}""");shot('yuri_name_glow')
 leakage=pg.evaluate("""()=>{ctx.save();ctx.globalAlpha=.37;ctx.shadowBlur=2;ctx.shadowColor='#aabbcc';ctx.imageSmoothingEnabled=true;pilotNameDraw('YURI',240,240,16,'#ef4040',.8);const result=ctx.globalAlpha===.37&&ctx.shadowBlur===2&&ctx.imageSmoothingEnabled===true;ctx.restore();return result;}""")
 check(leakage,'name glow restores alpha, shadow and smoothing state for subsequent body text')
 colors=pg.evaluate("""()=>PILOTS.map(p=>{pilotNameDraw(p.name,240,100,22,p.tint,1);const entry=pilotNamePlates.get(p.name.toUpperCase()+'|22|'+p.tint);const d=entry.rim.getContext('2d').getImageData(0,0,entry.rim.width,entry.rim.height).data;let pixel=null;for(let i=0;i<d.length;i+=4)if(d[i+3]===255){pixel=[d[i],d[i+1],d[i+2]];break;}return {pilot:p.key,tint:p.tint,pixel};})""")
 check(len(colors)==9 and all(r['pixel'] and ''.join('%02x'%v for v in r['pixel'])==r['tint'].lstrip('#').lower() for r in colors),'each pilot name uses its exact signature color in the rendered edge pixels')
 ...
 ff=imageio_ffmpeg.get_ffmpeg_exe();movie=O/'Pilot_Name_Glow_0914.mp4'
