...
R=Path(__file__).resolve().parents[2];O=R/'_shots/furyship_solid_0914';O.mkdir(parents=True,exist_ok=True)
...
with sync_playwright()as p:
 ...
 window.__partAudit=[];window.__cloudAudit=[];
 const cloud=furyIntroClouds;furyIntroClouds=function(S){const d=ctx.drawImage;ctx.drawImage=function(){if(S.t>10&&!S.space)__cloudAudit.push([S.t,...Array.from(arguments).slice(-4)]);return d.apply(this,arguments);};try{return cloud.apply(this,arguments);}finally{ctx.drawImage=d;}};
 window.__partWidths=[];const blit=furyShipBlit;furyShipBlit=function(k,x,y,w,h){if(FURY_KIT.some(q=>k.startsWith(q.key+'_'))){__partWidths.push(w);const m=ctx.getTransform();__partAudit.push([k,ctx.globalAlpha,arguments[6],m.a*m.d-m.b*m.c]);}return blit.apply(this,arguments);};}''')
 ...
 targets=[.1,3.3,3.8,4.3,4.9,5.5,6,7.2,8.5,10,11.9,13.5,15.6,17.2,18.5,19.5,20,24];white_shot=None
 ...
 ok(all(m['speed']==1000 for m in active),'constant 1000 px/s travel throughout sky, assembly, reveal and countdown')
 ok(all(abs((z['scroll']-a['scroll'])/(z['t']-a['t'])-1000)<.01 for a,z in zip(active,active[1:])if z['t']>a['t']),'actual background accumulator never decelerates or stops')
 ...
 audit=pg.evaluate('()=>({parts:__partAudit,clouds:__cloudAudit,cues:__alog.syn.filter(e=>e[1]==="furyPartArrival")})')
 ok(all(q[0].endswith('_top') and q[1]==1 and q[2]==1 and q[3]>0 for q in audit['parts']),'all actual part draws use one opaque top plate and no mirrored transform')
 cues=audit['cues'];ok(len(cues)==12 and [q[2][0]for q in cues]==list(range(12)),'twelve arrival sounds fire exactly once each, in part order')
 ok(all(31<=b[0]-a[0]<=34 for a,b in zip(cues,cues[1:])),'part arrivals are staggered by 0.54 seconds, not a simultaneous spawn')
 cloud=audit['clouds'];ok(bool(cloud)and all(abs((q[1]+q[3]/2)-(-56))<.01 or abs((q[1]+q[3]/2)-536)<.01 for q in cloud),'clearing has only side banks; no returning top/bottom cloud caps')
 ok(len(set(round(q[2]+q[4]/2,2)for q in cloud))==4,'all four side-bank heights remain fixed while the sky travels vertically')
 launch=pg.evaluate('()=>Array.from({length:12},(_,i)=>furyPartPose({phase:"drift",age:FURY_PART_START+i*FURY_PART_GAP+.000001,partStartAge:FURY_PART_START},i,240,302.08,118))')
 ok(all(q and q['y']>512+q['size']/2 for q in launch),'every piece begins wholly below the screen before rising into its orbit')
 (O/'part_audit.json').write_text(json.dumps({'arrivalCues':cues,'partDrawCount':len(audit['parts']),'keys':sorted(set(q[0]for q in audit['parts'])),'partOpacityValues':sorted(set(q[1]for q in audit['parts'])),'sideBankHeights':sorted(set(round(q[2]+q[4]/2,2)for q in cloud))},indent=2)+'\n')
 ...
 movie=O/'BulletsOfFury_Solid_Assembly_0914.mp4'
