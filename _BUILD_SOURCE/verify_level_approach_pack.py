...
PACK = ROOT / "assets" / "game" / "cinematic_level_approaches"
SIZE = (1672, 941)
...
def main() -> None:
    ...
    preview_path = PACK / "previews" / "level_approaches_contact.jpg"
    ...
    assert all(constraints.values()), f"global empty-scene constraints are not all enabled: {constraints}"
    ...
    assert set(zones["shared_recommended_zones"]) == {
        "foreground_full_cast",
        "left_actor",
        "center_actor",
        "right_actor",
        "dialogue_lower_third",
    }
    ...
    print("PASS: 9/9 level-approach masters verified at 1672x941 RGB.")
    print("PASS: manifest declares environment-only assets with zero baked entities, ships, enemies, characters, projectiles, text or UI.")
    print("PASS: all shared interaction zones remain within the native canvas.")
