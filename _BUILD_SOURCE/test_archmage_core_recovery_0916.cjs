module.exports=function testArchmageCoreRecovery(vm,ctxv,ok){
  ...
  const out=JSON.parse(vm.runInContext(`(function(){
    ...
    try{
      ...
      h.t=4.39;h.shotCd=.5;hammerBossTick(boss,.02);o.megaCharge=h.state==='mega_charge';hammerBossTick(boss,.40);const W=boss._combatWarnings&&boss._combatWarnings['archmage-fused-wave'];o.warning=!!W&&W.t===.4&&W.warm===1.65&&!W.released;combatWarningDraw=function(owner,q){calls.warn.push(q);};hammerBossDraw(boss);o.warningDraw=calls.warn.length===1&&calls.warn[0].progress===.4/1.65&&calls.warn[0].width===hammerEradWidth()&&calls.warn[0].ey===VH;
