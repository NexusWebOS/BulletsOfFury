...
module.exports=function(vm,ctxv,ok){
  ...
  const q=JSON.parse(vm.runInContext(`(function(){
    ...
    var expected={1:'razorback',2:'emberwing',3:'glacierlance',4:'furytalon',5:'voidreaver',6:'tempest',7:'ironserpent',8:'nighthammer',9:'solarwarden'},seen=[];
    try{
      for(var s=1;s<=9;s++){
        ...
        for(const k of ['easy','normal','hard','furious']){
          ...
          var waves=stagePlan.filter(w=>w.fn&&w.fn._difficultyElite),want=k==='furious'?2:(k==='hard'?1:0);
          o.counts=o.counts&&waves.length===want;o.normalClean=o.normalClean&&((k==='easy'||k==='normal')?waves.length===0:true);
          o.hardOne=o.hardOne&&(k!=='hard'||waves.length===1);o.furiousTwo=o.furiousTwo&&(k!=='furious'||waves.length===2);
      ...
      o.authored=Object.keys(DIFFICULTY_ELITE_STAGE).length===9&&Object.values(DIFFICULTY_ELITE_STAGE).flat().every(k=>ELITEX[k]);
