...
def main():
    ...
    with sync_playwright()as p:
        ...
        prep(1);pg.evaluate('()=>{window.__jets=[spawnEnemy("s1jetdelta",worldWidth()/2-65,100,{route:"straight"}),spawnEnemy("s1jetdelta",worldWidth()/2+65,140,{route:"straight"})];__jets.forEach(e=>{e.ghost=false;e.enter=false;e._volCd=0;});}')
        step(80);shot('s1_twin_guns');details['jetShots']=pg.evaluate('()=>eBullets.map(q=>({kind:q.kind,vx:q.vx,vy:q.vy}))')
        ok(bool(details['jetShots'])and all(q['kind']=='s1bullet'and abs(q['vx'])<1e-8 for q in details['jetShots']),'natural stage-1 jets fire straight paired gun lanes without extra spreads')
        pg.evaluate('()=>player.x=camLeftX()+30');step(25)
        ok(pg.evaluate('()=>eBullets.filter(q=>q.kind==="s1bullet").every(q=>Math.abs(q.vx)<1e-8)'),'existing jet gun rounds do not steer when the player changes column')
        ...
        fight(3,'boss');ok(ready('()=>["bmfx_fov_green_tall","bmfx_fov_yellow_tall","bmfx_fov_red_tall","l23fx_rime_laser_3"].every(k=>XART.rdy(k))'),'stage-3 FOV and authored laser charge art decode')
        phases=[]
        ...
        ok(all(set(q['slots']).issubset({'L','C','R'})and 'C'not in q['slots']and q['charge']['kind']=='centerPulse'for q in phases),'stage-3 held beams leave only side cannon tips while the central aperture charges a separate pulse')
        ...
        pg.evaluate('()=>{boss._xenoGrid=null;boss._sba=null;xenoRegentGridStart(boss,2);}');step(18);shot('s5_grid_first_warning');step(172);shot('s5_grid_next_warning')
        ...
        pg.evaluate('()=>{pBullets=[];window.__decorationBefore=explosions.length;for(let i=0;i<100;i++){const q={x:130+i%10*30,y:200+Math.floor(i/10)*25};pBullets.push(q);spaceImpact(q,"volley",5,90);}}')
        shot('s5_bounded_impacts');details['impactBudget']=pg.evaluate('()=>({visible:pBullets.filter(q=>!q._quietVisual).length,explosions:explosions.length-window.__decorationBefore})')
        ok(details['impactBudget']['visible']<=6 and details['impactBudget']['explosions']==0,'dense space volleys cannot pile opaque generic explosions over the playfield')
        fight(2,'boss');pg.evaluate('()=>{furnaceEnter(boss,"head");boss._fz.trans=0;boss._fz.idx=0;boss._fz.attack="eyeStab";boss._fz.at=.15;boss._fz.shotBeat=-1;player.x=worldWidth()/2-70;}')
        step(8);shot('s2_head_green');step(22);shot('s2_head_yellow');step(48);shot('s2_head_red')
        details['headBefore']=pg.evaluate('()=>({eyes:[boss._fz.eye[-1],boss._fz.eye[1]],x:boss.x,y:boss.y})')
        pg.evaluate('()=>player.x=camRightX()-30');step(18);shot('s2_head_release')
        details['headAfter']=pg.evaluate('()=>({eyes:[boss._fz.eye[-1],boss._fz.eye[1]],x:boss.x,y:boss.y,beams:boss._fz.beams.length})')
        ok(details['headBefore']['eyes']==details['headAfter']['eyes'] and details['headBefore']['x']==details['headAfter']['x'] and details['headAfter']['beams']==2,'Furnace head locks its pose and eyes before release and cannot chase a late dodge')
        ok(pg.evaluate('()=>["green","yellow","red"].every(c=>window.__blits.some(q=>q.key==="bmfx_fov_"+c+"_tall"))'),'shared head warning renders all three authored FOV charge colors')
