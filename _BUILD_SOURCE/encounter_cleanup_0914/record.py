...
OUT=ROOT/'_shots/encounter_cleanup_0914/video';OUT.mkdir(parents=True,exist_ok=True)
TAKES=[('s1_guns_clean',1,None,6),('s3_cannon_lasers',3,'boss',7),('s2_head_locked',2,'boss',6),('s5_regent_and_volley',5,'boss',18)]
...
def main():
 ...
 final=OUT/'BulletsOfFury_Readable_Attacks_0914.mp4';subprocess.run([ff,'-y','-v','error','-f','concat','-safe','0','-i',str(inputs),'-c','copy','-movflags','+faststart',str(final)],check=True)
