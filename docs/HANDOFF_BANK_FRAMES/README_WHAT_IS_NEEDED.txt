...
view with the canopy still visible, 39 px wide where a true edge is 18.
...
----------------------------------------------------------------------------
  ...
  * As it rolls you should see the SIDE of the fuselage. That is what makes a left bank
    differ from a right one, and it is the whole reason these are worth drawing by hand.
  * Camera stays directly overhead. Same design, colours, panel detail, light from above.
...
SIZE DRIFT IS FINE. If the tool returns a frame a little off the target size, or slightly
restyled, the build absorbs it - frames are fitted to the reel height and aligned by
silhouette, not by their bounding box. What CANNOT be absorbed is the appearance drifting
BETWEEN frames of the same ship, because that makes the aircraft morph mid-lean. Keep one
ship consistent across its six frames and any overall size difference gets normalised.


WHY EACH SHIP IS ON THE LIST (measured, not opinion)
============================================================================
A DERIVED reel is a horizontal squash of the top view, and that squash is symmetric about
the roll angle - so its left and right poses are THE SAME PIXELS and it can never show
direction. Verified per ship by comparing l against r, and pv0 against pv4, byte for byte.
...
NOT ON THE LIST - authored, and they do carry direction:
  MAVERICK   l, pv0 and br1 are all 156 px - three roll angles sharing one width.
  FREEZER    br2 is 85 px against a 197 px hull (0.43) - not a true edge, and br2 and br3
             share a width.
...
JUGGERNAUT   - work from juggernaut_TOP_reference.png  (203x213)
----------------------------------------------------------------------------
  frame                    target size  what it is
  pv3  (roll 17 deg)       194x213      gentle bank right - you start to see its right side
  r  (roll 20 deg)         191x213      bank right - the standard lean
  pv4  (roll 27 deg)       181x213      hard bank right
  br7  (roll 45 deg)       144x213      rolling right - wings clearly foreshortened
  br6  (roll 90 deg)       16x213       KNIFE EDGE - fuselage spine and the razor edge of the wings only
  br5  (roll 135 deg)      144x213      past vertical - now showing the BELLY, still narrow
  the 135 deg frame shows the UNDERSIDE - use juggernaut_BELLY_reference.png as its source.
...
NOT NEEDED - the build already has these:
  level (0 deg) = the top reference;   180 deg = the belly reference;
...
THE TOP AND BELLY PLATES DOUBLE AS SHIP AVATARS. They are clean, flameless,
transparent-background renders of each aircraft at full detail, which is exactly what a
menu, a stat panel or a pilot card wants. Reuse them there rather than cutting new ones.