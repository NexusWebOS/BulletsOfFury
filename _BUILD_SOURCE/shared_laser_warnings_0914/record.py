...
OUT=ROOT/'_shots/shared_laser_warnings_0914/video';OUT.mkdir(parents=True,exist_ok=True)
TAKES=[('s3_cannon_lasers',3,'boss',7),('pause_flow',1,None,12)]
DRIVE=r'''([name,i])=>{const K=Input.keys;for(const k of ['a','d','w','s','j','h'])K[k]=false;
 if(name=='pause_flow'){if(i===240){playerHit=window.__realHit;playPauseChoose(1);}return;}
 ...
 if(name==='s1_guns_clean'&&i%120===0){for(const dx of [-85,85]){const e=spawnEnemy('s1jetdelta',worldWidth()/2+dx,-35,{route:'straight'});if(e)e.ghost=false;}}
 if(name==='s2_head_locked'){const tx=mid+Math.sin(i/60*1.7)*150;K.a=player.x>tx+4;K.d=player.x<tx-4;}
...
def main():
 ...
 with sync_playwright()as p:
  ...
  for name,stage,role,seconds in TAKES:
   ...
   for frame in range(seconds*30):
    ...
    if frame%60==0:
     q=pg.evaluate('()=>({state:state,dead:player.dead,error:window.__err||null,target:window.__tgt()&&window.__tgt().name,mode:boss&&boss._s4war&&boss._s4war.mode,launch:drawLaunch._phase,missiles:pBullets.filter(q=>q.kind==="spaceVolley").length})');assert (name=='pause_flow' or not q['dead'])and not q['error'],q;timeline.append({'seconds':frame/30,**q});print(name+' '+str(frame/30)+'s '+json.dumps(q),flush=True)
 ...
 final=OUT/'BulletsOfFury_Pause_and_Blue_Lasers_0914.mp4';subprocess.run([ff,'-y','-v','error','-f','concat','-safe','0','-i',str(inputs),'-c','copy','-movflags','+faststart',str(final)],check=True)
