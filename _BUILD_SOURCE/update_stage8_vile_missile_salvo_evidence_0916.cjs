...
const shot=path.join(R,'_shots','stage8_vile_missile_salvo_warning_0916','results.json');
const qa=path.join(R,'docs','qa','stage8_vile_missile_salvo_warning_0916.json');
const result=JSON.parse(fs.readFileSync(shot,'utf8'));
result.verification={focused:'section 347: 9/9',fullSuite:{passing:4412,failures:56,failureNames:'exact established 56-name baseline; no new names'},chromium:'16/16',pixelReview:'four native 960x1024 frames inspected'};
...
data.updated='2026-09-16';data.latestBatch={description:'Stage-8 FURIOUS DEATH committed four-lane straight-missile warning',evidence:'STAGE8_VILE_MISSILE_SALVO_WARNING_0916.md'};
fs.writeFileSync(checklist,(JSON.stringify(data,null,2)+'\n').replace(/\n/g,'\r\n'),'utf8');
console.log('UPDATED_STAGE8_VILE_MISSILE_SALVO_EVIDENCE_0916');
