...
panel='''<div style="position:fixed;left:0;top:0;z-index:99999;background:#142034;color:white;font:11px monospace;padding:6px;width:180px;max-height:80px;overflow:auto;opacity:.18">
<b>Authored controls / real renderer</b><br><select id="qaScreen"></select>
<button id="qaDraw">Render</button><button id="qaNext">Next pilot</button><button id="qaB">B</button><button id="qaDelete">Backspace</button><pre id="qaState"></pre></div>
<script>
...
function qaReport(){document.getElementById('qaState').textContent=JSON.stringify({state,back:'B action',backspace:'delete text only',pilot:PILOTS[pilotIndex].key,viewport:pilotViewSize(),backing:[cv.width,cv.height],fullscreen:document.body.classList.contains('cinematic-full'),rect:cv.getBoundingClientRect().toJSON(),roster:psLineupRects,pixel:Array.from(ctx.getImageData(400,300,1,1).data),alpha:ctx.globalAlpha,transform:ctx.getTransform().toString()},null,1);}
...
document.getElementById('qaNext').onclick=()=>{pilotIndex=(pilotIndex+1)%PILOTS.length;qaDraw();pcSkip();qaDraw();};sel.onchange=qaReset;document.getElementById('qaDraw').onclick=qaDraw;
...
q=R/'_shots/pilot_fullscreen_0914';q.mkdir(parents=True,exist_ok=True)
